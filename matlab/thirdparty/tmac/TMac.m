...
for n = 1:N
    coNway(n) = prod(Nway)/Nway(n); % number of elements in a slice along dimension n
end
nrmb = norm(data);
...
Out.rank = coreNway;
Out.best_test_err = Inf;
Out.test_err = nan(maxit, N+1);
...
for k = 1:maxit
    ...
    for n = 1:N
        if alpha(n) > 0
            ...
            M = M+alpha(n)*Mn;
            Out.test_err(k,n) = ComputeError(opts.T_testIdx, Mn(opts.testIdx));
        else
            Out.test_err(k,n) = NaN;
        end
    end
    ...
    if isfield(opts, 'T_testIdx') && isfield(opts, 'testIdx')
        est = M(opts.testIdx);
        true = opts.T_testIdx;
        Out.test_err(k, N+1) = ComputeError(true, est);
        if Out.test_err(k, N+1) < Out.best_test_err
            Out.best_test_err = Out.test_err(k, N+1);
            %Out.best_model = {X,Y};
        end
        %fprintf('\b\b\b\b\b\b\b\b\b\b%3i: %0.3f', k, Out.test_err(k, N+1));
        fprintf('\rIteration %3i: %0.3f %0.3f %0.3f %0.3f, ', k, Out.test_err(k, :));
    end
