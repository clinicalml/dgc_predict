...
# Compute mean gene-gene correlations across all cell types
...
# diag(meanG) = NA
# save(meanG, file=ResultsDir('large/mean_gene_gene_correlation.RData'))
load(ResultsDir('large/mean_gene_gene_correlation.RData'))
...
labelRange = list(drug="name %in% c('homoharringtonine', 'terfenadine','dexamethasone', 'JNJ-38877605','M3','carbetocin')", #GNF-2')", 
                  gene='y > 1',
                  cell="name %in% c('HUH7','NCIH508','SNUC5','HEK293T')")
dodge = list(drug=-10, gene=-5, cell=0.1)
...
for(dim in c('drug', 'gene', 'cell')){
  f = substring(dim, 1, 1)
  xlab = bquote('DNPP PCT'[.(f)]) 
  ylab = bquote('Tensor PCT'[.(f)])
  x = rev(C[[xm[1]]][[dim]])
  y = rev(C$tensor[[dim]])
  FancyScatter(x, y, main=sprintf('%s-specific accuracy', capitalize(dim)), 
