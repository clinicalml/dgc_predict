...
for(model in c('asmatrix', 'constrained', 'si_lrtc', 'ha_lrtc', 'fa_lrtc','tmac','mean','mean2','knnd')){
  print(model)
  out = readMat(DataDir(sprintf('/results/tsize/small/benchmarking/%s_full_small_tensor.mat', model)))
...
results$model = revalue(as.factor(results$model), c('mean'='1D-Mean', 'mean2'='2D-Mean', 'knnd'='KNN',
                                                    'fa_lrtc'='FaLRTC','si_lrtc'='SiLRTC','ha_lrtc'='HaLRTC',
                                                    'asmatrix'='AsMatrix','constrained'='Constrained','tmac'='Tmac'))
#levels(results$model) = levels(results$model)

text_size = 16

res1 = subset(results, type=='PCTf')
res1 = transform(res1, model=reorder(model, value))
p1 = ggplot(res1) + theme_bw() + theme(text = element_text(size=text_size)) +
...
#res2 = transform(res2, model=reorder(model, value))
res2$model = reorder(res2$model,as.numeric(res1$model),order=TRUE)
p2 = ggplot(res2) + theme_bw() + theme(text = element_text(size=text_size)) +
  geom_boxplot(aes(x=model, y=value, group=model)) + ggtitle('Runtime') + xlab('') + ylab('sec. per fold') +
