...
# save(AUC, file=ResultsDir('small/choose_tensorDEG_method.RData'))

load(ResultsDir('small/choose_tensorDEG_method.RData'))

AUC$tcMethod = revalue(AUC$tcMethod, c('mean'='1D-Mean', 'mean2'='2D-Mean', 'knn' = 'DNPP', 'tensor'='Tensor'))
...
p1 = ggplot(auc, aes(x=percDEG, y=value, 
  ...
  ylab('AUC') +
  theme_classic() +
  theme(text=element_text(size=22), legend.position='bottom', legend.box='vertical') +
