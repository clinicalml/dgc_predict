...
# ATC codes
ATC = read.table(DataDir('atc/top_3_atc_codes_in_large_tensor.csv'), sep=',', header=TRUE, row.names=1)
...
Y = transform( merge(Y, TG, by=0, all=TRUE), row.names=Row.names, Row.names=NULL)
Y = transform( merge(Y, ATC, by=0, all=TRUE), row.names=Row.names, Row.names=NULL)
Y = Y[intersect(rownames(Y), rownames(X[[1]]$full)),]
