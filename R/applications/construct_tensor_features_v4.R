...
library(rhdf5)
# This differs from the previous version in that I'm adding more cell lines, and
# also adding a reduced-dimension version of that concatenated features
# ('TensorPCA features')
...
# Concatenated
L$tensor_cat$full = UnfoldTensor(tensors$comp, dim=1)

# Concatenated with reduced dimensionality
out = prcomp(L$tensor_cat$full, retx=TRUE, center=TRUE, scale.=FALSE)
cs = cumsum(out$sdev^2/sum(out$sdev^2))
L$tensor_pca200$full = out$x[,1:200] # The first 200 accounts for 69.12% of the variance
L$tensor_pca978$full = out$x[,1:978] # The first 978 accounts for 94.17% of the variance
...
dn = unlist(lapply(L[setdiff(names(L), c('tensor_cat', 'tensor_pca200', 'tensor_pca978'))], function(x) lapply(x, function(xx) dimnames(xx))), recursive=FALSE)
...
### Write to file
save(L, file=DataDir(sprintf('expr/drug/tensor_features_for_drug_property_prediction_15cells.RData', nCells)))
